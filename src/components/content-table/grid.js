...
import {
    requestTable, toggleFavoriteCurrency,
    getFavoritesCarrencyList, getUnremarkableCurrencyList, getDataByCarrentDate
} from '../../ducks/table';
...
const CarrencyList = ({ list, isFavorites }) => {
    ...
    return (
        list.map(({ code, title, rate }) => (
            <tr key={code}>
                <th scope="row">{code}</th>
                <td>{title}</td>
                <td>{rate}</td>
                <td>
                    <button className="ContentTable-action" onClick={() => { dispatch(toggleFavoriteCurrency(code)); }}>
