...
export const updateList = (listOfCurrencyList => createAction(UPDATE_LIST, { listOfCurrencyList }));

export const requestList = () => dispatch => {
    dispatch(startLoading());

    let promiseList = initializeList.map(({ valcode, date }) => api.headingForCurrenDateAndCurrentCurrency(valcode, date));
...
const normilizeList = list => list.map(({ exchangedate, rate }) => ({ date: exchangedate, rate }))
